...
- **Collection Deviation** — the only date-remap mechanism; an applicable Approved/Notified deviation outranks calendar filtering. *Removed from the prototype 2026-09-03 — no date-remap mechanism remains; holidays are skipped.*
...
| 3 | Stop selection | Declarative: matched by waste fraction + vehicle type | **Converged (issue #19, 2026-08-30)**: declarative stop-matching rule (fractions + optional vehicle type inside the scheme's planning area), resolved at generation time; explicitly picked per-day lists remain as the manual, small-scale mode |
...
| 6 | Cadence as catalogue/pricing dimension | The same Pickup Setting record is **required** on container prices and linked from products | **Converged (issue #20, 2026-08-31)**: a typed, reusable, project-scoped frequency record (`lib/data/service-frequencies.ts`) referenced by container and product; the container fact is derived from it and agreements display what they inherit. The price engine still omits it as a condition (conscious cut, extendable) |
...
| Pickup Setting — catalogue/pricing attribute | Typed frequency record (`lib/data/service-frequencies.ts`) referenced by container (`submittedValues.serviceFrequencyId`) and product, derived onto the "Service frequency" fact and displayed inherited on agreements — implemented for issue #20 (2026-08-31) at the research-corrected home | No longer a gap in shape. Vocabulary carries the real interval model (collections/week + weeks/days-between; every-N-weeks expressible but unseeded) plus a `schemeFrequency` mapping for issue #21; the prototype's `monthly` keeps no real-vocabulary counterpart. Price-condition slot stays consciously cut |
...
| Route Scheme — vehicle/driver rules + collection mode | Scheme assignment defaults (vehicle, driver, depot, unloading station, service provider) + dispatch-time overrides preserved on refresh | Collection mode (collect vs replace containers) has no prototype counterpart; otherwise no |
...
| Stop matching by fraction + vehicle type | Stop-matching rule on the scheme (`stopSelection: "rule"` + fractions + optional vehicle type, scoped to the planning area), resolved at generation by `lib/route-schemes/matching.ts` — implemented for issue #19 (2026-08-30); explicit `containerIds` / `containersByDay` remain as the manual mode | No longer a gap. The prototype adds an explicit geography dimension (planning area) the public real model doesn't document, and maps the real Active-agreement gate onto container-status eligibility (only Available containers match, exclusions shown with reasons) |

Reading the table: the prototype did not delete the Pickup Setting layer's *content* — it split it. The routing rules moved onto the scheme; the attribute/pricing role was renamed "Service frequency" and now has its typed home (open question 2, answered by research and implemented for issue #20). The Collection Calendar split differently than the repo previously recorded: its pattern+anchor half moved into scheme recurrence, its *materialized Days* half has no prototype counterpart at all (replaced by derivation + deviations), and the prototype's validity-filter calendar is largely a **new** concept (working days/holiday lists exist nowhere public in the real model). What the prototype still lacks outright: editable date records, every-N-weeks scheme recurrence (the frequency catalog's interval shape can express the promise, but scheme generation cannot serve it), collection mode. Declarative stop matching converged with issue #19.
...
4. **The reconciliation validation** (scheme recurrence vs promised service frequency — the deferred "week-parity vs pickup settings" class) becomes a first-class check *once step 2 gives the promise a typed home*: a scheme whose recurrence under-serves a linked container's promised frequency warns at validation. With the real vocabulary in view, "under-serves" generalizes to interval comparison (collections/week and weeks-between-N order naturally); the treatment of over-service is part of the decision, not assumed here. *Implemented (issue #21, 2026-08-31)*: `validateScheme` takes a pre-resolved `frequencyReconciliation` input (the wizard resolves every linked container's promise via `schemeFrequencyPromiseOfRecord`, both stop-selection modes) and compares on the collections-per-week scale — `RECURRENCE_WEEKLY_RATES` on the scheme side (monthly = 12/52), `promisedCollectionsPerWeek` from the interval vocabulary on the promise side, so every-N-weeks promises order without a scheme-cadence counterpart. **Over-service decided**: it also warns — scheme-vs-promise drift is usually a configuration or cost error worth surfacing — but with its own phrasing, after the under-served (promise-breaking) warnings. Both directions are non-blocking, like calendar warnings; on-demand promises are skipped. Harness-covered in `route-scheme-validation-harness.ts` and `service-frequency-harness.ts`.
