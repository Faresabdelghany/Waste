...
- **Collection Deviation** — an approved replacement of one planned service date with another, preserving the original service promise. *Removed 2026-09-03: the module, its form, the generation date-remap, and the portal notice banner were deleted; holiday and non-working dates are skipped, never moved.*

Runtime flow: Area/Zone + Calendar → Route Scheme → candidate service dates → calendar validity filtering → generated Routes. *(The Approved Deviations step was removed 2026-09-03.)*
...
| Q11 | `/plan` = Collection Deviations, Collection Calendars, Areas & Zones. Route Schemes stays in Route Studio; Vehicle Planning stays in Fleet (existing filtering preserved). *Superseded 2026-09-03: Collection Deviations removed — `/plan` = Collection Calendars, Areas & Zones.* |
| Q12 | Collection Calendars' add-deviation action form deleted; replaced by a real create-calendar form. Deviations creatable **only** via the Collection Deviations module. *Superseded 2026-09-03: the Collection Deviations module and its form were removed; nothing creates deviations.* |
| Q13 | Deviation `calendarId` authoritative: a deviation affects only schemes referencing the same calendar; legacy deviations without `calendarId` fall back to project-overlap. *Retired 2026-09-03 with the module.* |
...
| Q15 | Glossary updated: Collection Calendar redefined (deviations removed from it), Collection Deviation + Planning Area added, Pickup Setting / Collection Week retired to Avoid lists. *Amended 2026-09-03: Collection Deviation removed from the glossary and added to the Collection Calendar Avoid list.* |
