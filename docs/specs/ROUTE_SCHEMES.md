...
- **FR-2**: Guided Setup steps (validated as prototype variant A):
  ...
  5. **Route map** — map preview of the generated routes: one colored route line per service day with stop pins, an All-days/per-day filter, and a legend with each day's stop count and fraction mix.
